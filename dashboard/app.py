...
def page_2fa():
    ...
    with col:
        if st.session_state.get("show_code"):
            st.warning(f"🔑 Code de vérification : **{st.session_state['show_code']}**")
        st.markdown(f"""
        <div style="text-align:center;padding:40px 0 20px 0;">
...
def render_sidebar(user):
    ...
    with st.sidebar:
        ...
        source = "📊 Dataset exemple"
        seal = True
        if can_access(user["role"], "view_all_transactions"):
            st.subheader("📂 Données")
            source = st.radio("Source", ["📊 Dataset exemple", "📁 Uploader un fichier", "🗄️ Historique"], label_visibility="collapsed")
...
def main():
    ...
    filepath = None
    filepath = None
    if source == "📊 Dataset exemple":
        sp = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "sample_transactions.csv")
        if os.path.exists(sp): filepath = sp
        ...
        if up:
            with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(up.name)[1]) as tmp:
              tmp.write(up.read()); filepath = tmp.name
